...
async def process_age(message: types.Message, state: FSMContext):
    ...
    age = int(age)
    if age <= 17:
        await message.answer("Возраст должен быть не меньше 17 лет!")
        return
...
async def process_occupation(message: types.Message, state: FSMContext):
    ...
    await message.answer("Укажите свою заработную плату")
...
async def process_salary(message: types.Message, state: FSMContext):
    ...
    await database.execute("""
               INSERT INTO review_results (name, age, occupation, salary)
               VALUES (?, ?, ?, ?)""",
